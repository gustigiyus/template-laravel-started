<x-app-layout>
    <div>
        <div class="flex justify-between items-center mb-5">
            <h1 class="text-xl font-semibold text-gray-900 sm:text-xl dark:text-white">Menus</h1>
            ...
            <table id="myTableMenus"
                ...
                <thead
                    ...
                    <tr>
                        ...
                        <th class="w-[10%] p-2">Menu Name</th>
                        <th class="p-2">Menu Description</th>
    ...
    @include('pages.utility.menus.modal')
    ...
    @push('myscript')
        <script type="module">
            $(document).ready(function() {
                ...
                // Init Datatables
                var table = $('#myTableMenus').DataTable({
                    processing: true,
                    serverSide: true,
                    pageLength: 5,
                    lengthMenu: [
                ...
                setTimeout(() => {
                    ...
                    $('#myTableMenus thead tr th').addClass('p-4')
                    $('#myTableMenus tbody tr').addClass('border-b')

                    // Footer Table
                    $('#myTableMenus_info').closest('.grid.grid-cols-2.gap-4.mb-4').addClass(
                        'flex justify-between p-2 items-center text-sm')
                    $('#myTableMenus_info').closest('.grid.grid-cols-2.gap-4.mb-4').removeClass(
                ...
                $('body').on('click', '.btn-edit', function(e) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "{{ route('menuShow', ['id' => 'ID']) }}".replace('ID', id),
                        headers: {
                        ...
                        success: function(response) {
                            ...
                            $('#menu_desc').val(response.data.menu_desc)
                            actionMode = 'Update';
                            editId = id;
                            modal1.show();
                        }
                ...
                function save(action, id) {
                    ...
                    let menu_desc = $('textarea[name="menu_desc"]').val()
                    ...
                    let url_data = '';
                    if (action == 'Save') {
                        url_data = "{{ route('menuStore') }}";
                    } else if (action == 'Update') {
                        url_data = "{{ route('menuUpdate', ['id' => 'ID']) }}".replace('ID', id);
                    } else {
                        return false
                    }
                    ...
                    $.ajax({
                        ...
                        error: function(response) {
                            ...
                            if (res.menu_name) {
                                $('.msg-alert-menu_name').text(res.menu_name).addClass(
                                    'message-error')
                                $('.input-alert-menu_name').addClass('input-error').removeClass(
                                    'input-success input-normal')
                                $('.label-alert-menu_name').addClass('label-error').removeClass(
                                    'label-success label-normal')
                            } else {
                                $('.msg-alert-menu_name').text('').removeClass('message-error')
                                $('.input-alert-menu_name').addClass('input-success').removeClass(
                                    'input-normal input-error')
                                $('.label-alert-menu_name').addClass('label-success').removeClass(
                                    'label-normal label-error')
                            }

                            if (res.menu_desc) {
                                $('.msg-alert-menu_desc').text(res.menu_desc).addClass(
                                    'message-error')
                                $('.input-alert-menu_desc').addClass('input-error').removeClass(
                                    'input-success input-normal')
                                $('.label-alert-menu_desc').addClass('label-error').removeClass(
                                    'label-success label-normal')
                            } else {
                                $('.msg-alert-menu_desc').text('').removeClass('message-error')
                                $('.input-alert-menu_desc').addClass('input-success').removeClass(
                                    'input-normal input-error')
                                $('.label-alert-menu_desc').addClass('label-success').removeClass(
                                    'label-normal label-error')
                            }
                        }
                    });
