<x-app-layout>
            ...
            <table id="myTableRoles"
                ...
                <thead
                    ...
                    <tr>
                        ...
                        <th class="w-[10%] p-2">Role Name</th>
                        <th class="p-2">Role Description</th>
    ...
    @include('pages.utility.roles.modal')
    ...
    @push('myscript')
        <script type="module">
            $(document).ready(function() {
                ...
                // Init Datatables
                var table = $('#myTableRoles').DataTable({
                    processing: true,
                    serverSide: true,
                    pageLength: 5,
                    lengthMenu: [
                ...
                setTimeout(() => {
                    ...
                    $('#myTableRoles thead tr th').addClass('p-4')
                    $('#myTableRoles tbody tr').addClass('border-b')

                    // Footer Table
                    $('#myTableRoles_info').closest('.grid.grid-cols-2.gap-4.mb-4').addClass(
                        'flex justify-between p-2 items-center text-sm')
                    $('#myTableRoles_info').closest('.grid.grid-cols-2.gap-4.mb-4').removeClass(
                ...
                $('body').on('click', '.btn-edit', function(e) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "{{ route('roleShow', ['id' => 'ID']) }}".replace('ID', id),
                        headers: {
                        ...
                        success: function(response) {
                            ...
                            $('#role_desc').val(response.data.role_desc)
                            actionMode = 'Update';
                            editId = id;
                            modal1.show();
                        }
                ...
                function save(action, id) {
                    ...
                    let role_desc = $('textarea[name="role_desc"]').val()
                    ...
                    let url_data = '';
                    if (action == 'Save') {
                        url_data = "{{ route('roleStore') }}";
                    } else if (action == 'Update') {
                        url_data = "{{ route('roleUpdate', ['id' => 'ID']) }}".replace('ID', id);
                    } else {
                        return false
                    }
                    ...
                    $.ajax({
                        ...
                        error: function(response) {
                            ...
                            if (res.role_name) {
                                $('.msg-alert-role_name').text(res.role_name).addClass(
                                    'message-error')
                                $('.input-alert-role_name').addClass('input-error').removeClass(
                                    'input-success input-normal')
                                $('.label-alert-role_name').addClass('label-error').removeClass(
                                    'label-success label-normal')
                            } else {
                                $('.msg-alert-role_name').text('').removeClass('message-error')
                                $('.input-alert-role_name').addClass('input-success').removeClass(
                                    'input-normal input-error')
                                $('.label-alert-role_name').addClass('label-success').removeClass(
                                    'label-normal label-error')
                            }

                            if (res.role_desc) {
                                $('.msg-alert-role_desc').text(res.role_desc).addClass(
                                    'message-error')
                                $('.input-alert-role_desc').addClass('input-error').removeClass(
                                    'input-success input-normal')
                                $('.label-alert-role_desc').addClass('label-error').removeClass(
                                    'label-success label-normal')
                            } else {
                                $('.msg-alert-role_desc').text('').removeClass('message-error')
                                $('.input-alert-role_desc').addClass('input-success').removeClass(
                                    'input-normal input-error')
                                $('.label-alert-role_desc').addClass('label-success').removeClass(
                                    'label-normal label-error')
                            }
                        }
                    });
